...
class MyWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    startupInitialized = QtCore.pyqtSignal()

    def _shouldLoadWindowIcon(self) -> bool:
        return not (sys.platform == "darwin" and getattr(sys, "frozen", False))

    def __init__(self, *args, obj=None, **kwargs) -> None:
        super(MyWindow, self).__init__(*args, **kwargs)
        self._startupInitialized = False
        if self._shouldLoadWindowIcon():
            self.setWindowIcon(QtGui.QIcon(f"{utils.assets_path()}rsconvert.png"))
            startup.mark("window icon loaded")
        else:
            startup.mark("window icon skipped for packaged macOS build")
        self.setupUi(self)
        startup.mark("setupUi complete")
        try:
        ...
        startup.mark("settings handler created")
        ...
        startup.mark("window init complete")

    def initializeStartupState(self) -> None:
        if self._startupInitialized:
            return
        self._startupInitialized = True
        startup.mark("deferred startup begin")
        self.settingsHandler.loadSettings(self)
        ...
        self.syncUiState()
        self.startupInitialized.emit()
        startup.mark("deferred startup complete")

    def syncUiState(self) -> None:
    ...
    def closeEvent(self, event: QtGui.QCloseEvent) -> None:
        self.settingsHandler.saveSettings(self)
