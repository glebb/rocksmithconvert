...
def test_invoking_setPlatform_works(widget: rocksmithconvert.controllers.MainWindowController, qtbot):
	widget.window.comboBoxPlatform.setCurrentIndex(2)
	widget.processFiles(["test_p.psarc"])
	model: ProcessModel = widget.convertService.process.call_args.args[0]
	assert model.targetPlatform == "PC"

def test_autoprocess_works(widget: rocksmithconvert.controllers.MainWindowController, qtbot):
